...
export function TriggersScreen({ agentId }: TriggersScreenProps) {
  ...
  const filteredRunningTriggers = useMemo(
    () =>
      runningTriggers.filter(
        (trigger) =>
          trigger.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          trigger.description?.toLowerCase().includes(searchQuery.toLowerCase()),
      ),
    [runningTriggers, searchQuery],
  );

  const filteredPausedTriggers = useMemo(
    () =>
      pausedTriggers.filter(
        (trigger) =>
          trigger.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
          trigger.description?.toLowerCase().includes(searchQuery.toLowerCase()),
      ),
    [pausedTriggers, searchQuery],
  );
  ...
  return (
    <div className="flex-1 overflow-auto pb-6">
      <div className="flex items-center justify-between pb-4 px-1 pt-1">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="default" size="sm" className="h-10 px-4 rounded-xl gap-2">
          ...
          <DropdownMenuContent align="end" className="w-72">
            <DropdownMenuItem onClick={() => setTriggerDialogType('schedule')} className="rounded-lg">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Schedule a trigger to run at a specific time</span>
              </div>
            </DropdownMenuItem>
            <DropdownMenuItem onClick={() => setTriggerDialogType('event')} className="rounded-lg">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">Make a trigger to run when an event occurs</span>
              </div>
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <div className="px-1">
        {filteredRunningTriggers.length === 0 && filteredPausedTriggers.length === 0 ? (
          <div className="text-center py-12 px-6">
            <div className="mx-auto w-12 h-12 bg-muted rounded-full flex items-center justify-center mb-4 border">
              <Zap className="h-6 w-6 text-muted-foreground" />
            </div>
            <h4 className="text-sm font-semibold text-foreground mb-2">No triggers configured</h4>
          ...
          <>
            {filteredRunningTriggers.length > 0 && (
              <section className="mb-6">
                <h2 className="text-sm font-medium mb-3">Running</h2>
                <div className="space-y-2">
                  {filteredRunningTriggers.map((trigger) => (
                    <TriggerCard
            ...
            {filteredPausedTriggers.length > 0 && (
              <section>
                <h2 className="text-sm font-medium mb-3">Paused</h2>
                <div className="space-y-2">
                  {filteredPausedTriggers.map((trigger) => (
                    <TriggerCard
                      key={trigger.trigger_id}
                      trigger={trigger}
                      onToggle={() => handleToggleTrigger(trigger)}
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete &quot;{triggerToDelete?.name}&quot;? This action cannot be undone and will
              stop all automated runs from this trigger.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <div className="flex items-center justify-between p-5">
        <div className="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground truncate">{trigger.description || 'No description'}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 ml-4">
          ...
          >
            {trigger.is_active ? <Pause className="h-7 w-7" /> : <Play className="h-5 w-5 ml-0.5" />}
          </Button>
