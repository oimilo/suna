...
type SubscriptionResponse = Awaited<ReturnType<typeof getSubscription>> & {
...
export function BillingModal({ open, onOpenChange, returnUrl = typeof window !== 'undefined' ? window?.location?.href || '/' : '/', showUsageLimitAlert = false }: BillingModalProps) {
    ...
    const getEffectiveCancellationDate = () => {
        if (subscriptionData?.subscription?.cancel_at) {
            return formatDate(normalizeTimestamp(subscriptionData.subscription.cancel_at));
        }

        return formatDate(normalizeTimestamp(subscriptionData?.subscription?.current_period_end));
    };

    const handleManageSubscription = async () => {
        try {
            setIsManaging(true);
            const response = await createPortalSession({ return_url: returnUrl });
            const redirectUrl = response.portal_url || (response as any).url;
            if (!redirectUrl) {
                                                ...
                                                <DialogDescription className="text-sm">
                                                    {commitmentInfo?.has_commitment && !commitmentInfo?.can_cancel ? (
                                                        ...
                                                        <>
                                                            ...
                                                            {formatDate(normalizeTimestamp(subscriptionData.subscription.current_period_end))}. 
