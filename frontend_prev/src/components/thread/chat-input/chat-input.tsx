...
export const ChatInput = memo(forwardRef<ChatInputHandles, ChatInputProps>(
  (
    ...
    const [prophetAgentModes, setProphetAgentModes] = useState<'adaptive' | 'autonomous' | 'chat'>('adaptive');
    ...
    const prophetAgent = agents.find(agent => agent.metadata?.is_prophet_default === true);
    const isProphetAgent = isLoadingAgents 
        ? true // Show Prophet modes while loading
        : (selectedAgent?.metadata?.is_prophet_default || (!selectedAgentId && prophetAgent !== undefined) || false);
    ...
    const renderControls = useMemo(() => (
      <div className="flex items-center justify-between mt-0 mb-1 px-2">
        <div className="flex items-center gap-3">
          ...
          {ENABLE_SUNA_AGENT_MODES && (isStagingMode() || isLocalMode()) && isProphetAgent && (
            <TooltipProvider>
              <div className="flex items-center gap-1 p-0.5 bg-muted/50 rounded-lg">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={() => setProphetAgentModes('adaptive')}
                      className={cn(
                        "p-1.5 rounded-md transition-all duration-200 cursor-pointer",
                        prophetAgentModes === 'adaptive'
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={() => setProphetAgentModes('autonomous')}
                      className={cn(
                        "p-1.5 rounded-md transition-all duration-200 cursor-pointer",
                        prophetAgentModes === 'autonomous'
                          ? "bg-background text-foreground shadow-sm"
                          : "text-muted-foreground hover:text-foreground hover:bg-background/50"
                      )}
                    >
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      onClick={() => setProphetAgentModes('chat')}
                      className={cn(
                        "p-1.5 rounded-md transition-all duration-200 cursor-pointer",
                        prophetAgentModes === 'chat'
                          ? "bg-background text-foreground shadow-sm"
                          : "text-muted-foreground hover:text-foreground hover:bg-background/50"
                      )}
                    >
                ...
                <TooltipTrigger asChild>
                  ...
                  >
                    {((loading || isUploading) && !isAgentRunning) ? (
                      <MiloLoader size="small" customSize={20} variant={buttonLoaderVariant} />
                    ) : isAgentRunning ? (
    ...
    ), [hideAttachments, loading, disabled, isAgentRunning, isUploading, sandboxId, projectId, messages, isLoggedIn, renderConfigDropdown, planModalOpen, setPlanSelectionModalOpen, handleTranscription, onStopAgent, handleSubmit, value, uploadedFiles, selectedMode, onModeDeselect, handleModeDeselect, isModeDismissing, isProphetAgent, prophetAgentModes, pendingFiles, threadId, selectedModel, googleDriveIcon, slackIcon, notionIcon, buttonLoaderVariant]);
                    ...
                    {isUploading && pendingFiles.length > 0 && (
                      <div className="absolute inset-0 bg-background/50 backdrop-blur-sm rounded-xl flex items-center justify-center">
                        <div className="flex items-center gap-2 bg-background/90 px-3 py-2 rounded-lg border border-border">
                          <MiloLoader size="small" customSize={16} variant="auto" />
