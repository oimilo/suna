...
export const AgentAvatar: React.FC<AgentAvatarProps> = ({
  ...
  agent: propAgent,
  agentId,
  fallbackName = BRANDING.defaultAgentName,
  ...
  isProphetDefault: propIsProphetDefault,
  ...
  const isProphet = propIsProphetDefault ?? agent?.metadata?.is_prophet_default;
  ...
  if (!agent && !propIconName && !propIsProphetDefault && agentId) {
    return (
      <div
...
export const AgentName: React.FC<AgentNameProps> = ({
  agent: propAgent,
  agentId,
  fallback = BRANDING.defaultAgentName
}) => {
  const cachedAgent = useAgentFromCache(!propAgent && agentId ? agentId : undefined);
  const agent = propAgent || cachedAgent;
