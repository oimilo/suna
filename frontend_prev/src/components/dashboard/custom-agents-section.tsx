...
import { useMiloTeamTemplates, useInstallTemplate } from '@/hooks/react-query/secure-mcp/use-secure-mcp';
...
export function CustomAgentsSection({ onAgentSelect }: CustomAgentsSectionProps) {
  ...
  const { data: templates, isLoading, error } = useMiloTeamTemplates();
  ...
  const handleCardClick = (template: any) => {
    const marketplaceTemplate: MarketplaceTemplate = {
      ...
      is_milo_team: template.is_milo_team || false,
