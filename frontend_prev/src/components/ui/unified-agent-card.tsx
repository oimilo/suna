...
const CardAvatar: React.FC<{ 
  ...
  const isProphetAgent = data.metadata?.is_prophet_default === true;
...
const AgentBadges: React.FC<{ data: BaseAgentData, isProphetAgent: boolean }> = ({ data, isProphetAgent }) => (
  <div className="flex gap-1">
    {!isProphetAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!isProphetAgent && data.is_public && (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return <MarketplaceBadge isMiloTeam={data.is_milo_team} isOwner={isOwner} />;
        case 'template':
          return <TemplateBadge isPublic={data.is_public} />;
        case 'agent':
          return <AgentBadges data={data} isProphetAgent={isProphetAgent} />;
        default:
          return null;
      }
    };
