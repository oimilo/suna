    ...
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultProphetAgent = agents.find(agent => agent.metadata?.is_prophet_default);
            selectedId = defaultProphetAgent ? defaultProphetAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      isProphetAgent: (agents: Agent[]) => {
        ...
        const currentAgent = selectedAgentId 
          ...
          : null;
        return currentAgent?.metadata?.is_prophet_default || selectedAgentId === undefined;
      },
    }),
    {
