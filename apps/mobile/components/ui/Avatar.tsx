...
interface AvatarProps extends ViewProps {
  ...
  /** Use Milo symbol instead of icon (for SUNA agent) */
  useMiloSymbol?: boolean;
 ...
 * - Milo symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for Milo symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // Milo symbol always uses solid black bg with white icon
    if (useMiloSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useMiloSymbol ? 0 : (showBorder ? 1.5 : 0),
