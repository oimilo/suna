...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for Prophet vs custom agents."""
    ...
    # Handle Prophet agents with special logic
    if is_suna_default:
...
def _extract_suna_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for Prophet agents - always use central config with user customizations."""
    from core.suna_config import SUNA_CONFIG
    
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using Prophet central config for agent {agent_id}")
    
    # Start with central Prophet config
    config = {
