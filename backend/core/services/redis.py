...
def initialize():
    ...
    redis_use_tls = os.getenv("REDIS_USE_TLS", os.getenv("REDIS_SSL", "")).lower() == "true"

    if redis_url:
        logger.info("Initializing Redis connection pool via URL configuration")
        ...
        if redis_use_tls:
            pool_kwargs.update(
                {
                    "ssl": True,
                    "ssl_cert_reqs": None,  # Upstash and other managed providers close connection if certs are required
                }
            )
