...
class DistributedLock:
    """Cooperatively manage critical sections across multiple workers via Supabase locks."""
    ...
    async def acquire(self, wait: bool = False, wait_timeout: int = 30) -> bool:
        """Attempt to acquire the lock. Optionally wait with exponential backoff."""
        ...
        while True:
            try:
                ...
                logger.error(f"[LOCK] Error acquiring lock {self.lock_key}: {exc}")
                if not wait:
                    return False
    ...
    async def release(self) -> bool:
        ...
        try:
            ...
            logger.error(f"[LOCK] Error releasing lock {self.lock_key}: {exc}")
            return False
...
async def distributed_lock(lock_key: str, timeout_seconds: int = 300, wait: bool = True):
    """Context manager wrapper for acquiring/releasing a distributed lock."""
...
class RenewalLock:
    """Helpers specific to subscription renewal processing."""
    ...
    async def check_and_mark_renewal_processed(
        ...
        existing = (
            await client.from_("renewal_processing")
            .select("id, processed_by, credits_granted")
        ...
        try:
            ...
            logger.error(f"[RENEWAL TRACK] Failed to mark renewal processed: {exc}")
            return False


class WebhookLock:
    """Helpers to enforce idempotent webhook handling."""

    @staticmethod
    async def check_and_mark_webhook_processing(
        event_id: str, event_type: str, payload: Optional[dict] = None
    ) -> Tuple[bool, Optional[str]]:
        db = DBConnection()
        client = await db.client

        existing = (
            await client.from_("webhook_events")
        ...
        if existing.data:
            ...
            try:
                ...
                if "duplicate key" in str(exc).lower() or "unique" in str(exc).lower():
                    logger.warning(
                        f"[WEBHOOK] Race condition detected for event {event_id}, another process started first"
                    )
    ...
    @staticmethod
    async def mark_webhook_failed(event_id: str, error: str):
        db = DBConnection()
        client = await db.client
