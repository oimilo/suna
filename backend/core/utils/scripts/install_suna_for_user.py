...
Simple script to install the Prophet default agent for users by email address or account ID.
...
class SunaUserInstaller:
    ...
    async def install_for_email(self, email: str, replace: bool = False):
        ...
        print(f"🚀 Installing Prophet default agent...")
        ...
        if agent_id:
            print(f"✅ Successfully installed the Prophet default agent!")
            print(f"   🤖 Agent ID: {agent_id}")
            print(f"   👤 User: {email}")
            print(f"   📦 Account: {account_id}")
        else:
            print(f"❌ Failed to install Prophet default agent for {email}")
    
    async def install_for_account_id(self, account_id: str, replace: bool = False):
        print(f"🚀 Installing Prophet default agent for account: {account_id}")
        ...
        try:
            ...
            if agent_id:
                ...
                print(f"❌ Failed to install Prophet default agent for account {account_id}")
...
async def main():
    parser = argparse.ArgumentParser(
        description="Install Prophet default agent for a user by email or account ID",
    ...
    parser.add_argument('--replace', action='store_true', 
                       help='Replace existing Prophet default agent if present')
