    ...
    try:
        if config.ENV_MODE != EnvMode.LOCAL:
            ...
            if not limit_check['can_create']:
                error_detail = {
                    "message": (
                        f"Maximum of {limit_check['limit']} {trigger_type_str} triggers allowed for your plan. "
                        f"You already have {limit_check['current_count']} {trigger_type_str} triggers."
                    ),
                ...
                logger.warning(
                    f"Trigger limit exceeded for account {user_id}: "
                    f"{limit_check['current_count']}/{limit_check['limit']} {trigger_type_str} triggers"
                )
