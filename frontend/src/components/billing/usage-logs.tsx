...
const getModelAlias = (model: string): string => {
  ...
  // Haiku/Gemini models -> Agente padrão
  if (modelLower.includes('haiku') || modelLower.includes('gemini')) {
    return 'Agente padrão';
  }
  ...
  const calculateUsageSummary = () => {
    ...
    if (creditsData) {
      return {
        ...
        totalCredits: Math.floor(creditsData.credit_balance || 0),
        totalAvailable: Math.floor(creditsData.credit_balance || 0),
        percentageUsed: 0,
        dailyExpiresIn: undefined as unknown as string | undefined,
      };
    }
    ...
    return {
      usedCredits: Math.min(usedCredits, subscriptionCredits),
      subscriptionCredits,
      ...
      totalAvailable: subscriptionCreditsRemaining,
            ...
            <div className="h-2 bg-black/[0.04] dark:bg-white/[0.04] rounded-full overflow-hidden flex">
              ...
              <div className="flex-[15] h-full bg-black/[0.04] dark:bg-white/[0.04] relative overflow-hidden">
                ...
                <div 
                  ...
                  style={{ 
                    width: summary.dailyCredits > 0 ? `${(summary.dailyCreditsRemaining / summary.dailyCredits) * 100}%` : '0%'
                  }}
                />
              </div>
            </div>
