...
export const EventTriggerConfigForm: React.FC<EventTriggerConfigFormProps> = ({
  ...
  const variableSpecs = useMemo(() => {
    if (!selectedWorkflow) return [] as VariableSpec[];
    ...
    return (child?.config?.playbook?.variables as VariableSpec[]) || [];
  }, [selectedWorkflow]);
              ...
              <div>
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Tabs value={config.is_active ? 'enabled' : 'disabled'} onValueChange={(value) => onChange({ ...config, is_active: value === 'enabled' })} className="w-full">
