  ...
  return (
    <Card className="gap-0 flex border shadow-none border-t border-b-0 border-x-0 p-0 rounded-none flex-col overflow-hidden bg-card">
      ...
      <CardContent className="p-4 space-y-3">
        {assistantContent && <div className="text-sm text-foreground mb-3">{assistantContent}</div>}

        {callsData.calls.length === 0 ? (
          <div className="text-sm text-muted-foreground text-center py-6">No calls found</div>
        ) : (
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-2 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs font-mono text-muted-foreground mt-1 truncate">{call.call_id}</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {callsData.message && (
          <div className="text-xs text-muted-foreground pt-2 border-t border-border">{callsData.message}</div>
        )}
      </CardContent>
    </Card>
  );
