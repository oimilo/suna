...
// Fallback: se o modal especializado não existir neste fork, não bloquear build
const TipTapDocumentModal = ({ open, onOpenChange }: any) => null as any;
type ExportFormat = 'docx' | 'txt';
const exportDocument = async ({ content, fileName, format }: { content: string; fileName: string; format: ExportFormat }) => {
  const blob = new Blob([
    format === 'txt' ? content : `<!DOCTYPE html><html><body>${content}</body></html>`
  ], { type: format === 'txt' ? 'text/plain' : 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' });
