  ...
  const MODEL_OPTIONS = useMemo(() => {
    ...
    if (!modelsData?.models || isLoadingModels) {
      ...
      models = modelsData.models.map(model => {
        ...
        const modelPriority = (model as any)?.priority ?? 0;
        
        return {
          id: shortName,
          label: displayName,
          requiresSubscription: (model as any)?.requires_subscription || false,
          priority: modelPriority,
          recommended: (model as any)?.recommended || false,
          top: modelPriority >= 90, // Mark high-priority models as "top"
          lowQuality: false, // All models in new system are quality controlled
          capabilities: (model as any)?.capabilities || [],
          contextWindow: (model as any)?.context_window || 128000
        };
      });
    }
