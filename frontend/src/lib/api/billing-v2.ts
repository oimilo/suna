...
export interface DeductResult {
  ...
  cost: number;
  new_balance: number;
  from_expiring?: number;
  from_non_expiring?: number;
...
export const billingApiV2 = {
  ...
  async getUsageHistory(days = 30) {
    const response = await backendApi.get('/billing/usage-history?days=' + days);
    if (response.error) throw response.error;
    return response.data!;
  },
  ...
  async getSubscriptionCancellationStatus() {
    const response = await backendApi.get<SubscriptionCancellationStatus>(
      '/billing/subscription-cancellation-status'
    );
    if (response.error) throw response.error;
    return response.data!;
  },
