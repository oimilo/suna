...
export const composioApi = {
  ...
  async getCredentialsProfiles(): Promise<ComposioToolkitGroup[]> {
    ...
    if (!response.success) {
      throw new Error(response.error?.message || 'Failed to load credential profiles');
    }
  ...
  async getMcpUrl(profileId: string): Promise<ComposioMcpUrlResponse> {
    ...
    if (!response.success) {
      throw new Error(response.error?.message || 'Failed to get MCP URL');
    }
  ...
  async getToolkitIcon(toolkitSlug: string): Promise<{ success: boolean; icon_url?: string }> {
    ...
    if (!response.success) {
      throw new Error(response.error?.message || 'Failed to get toolkit icon');
    }
    return {
