...
const getDefaultModel = (accessibleModels: ModelOption[]): string => {
  ...
  const basicModel = accessibleModels.find(m => m.id === 'prophet/basic');
  if (basicModel) return basicModel.id;
  
  const powerModel = accessibleModels.find(m => m.id === 'prophet/power');
...
export const useModelSelection = () => {
  ...
  useEffect(() => {
    ...
    if (wasFree && isNowPaid && prevTierKey.current !== null) {
      ...
      const powerModel = availableModels.find(m => m.id === 'prophet/power' && !m.requiresSubscription);
      if (powerModel) {
        console.log('🚀 useModelSelection: Upgraded to paid tier! Switching to kortix/power');
        setSelectedModel('prophet/power');
      }
    }
